...
class Status extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cron/everflow:status {type}';
     ...
     * @var string
     */
    protected $description = 'Run status request';
    ...
    protected function request()
    {
        ...
        $dataRequest = modelRequestStatus::where('date', '<=', date('Y-m-d'))
        ...
        if($dataRequest){
            foreach($dataRequest as $iter){
                ...
                if($dataOffer->ef_id){
                    ...
                    $ef_resp = $ef_Offer->updateOfferStatus($iter);
                    if($ef_resp['updated']){
                        $iter->status = 3;
                    } else {
