    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <div class="box box-default">
                ...
                <form class="" method="post" action="{{ route('admin.request.cap.save.edit') }}">
                    <div class="box-body">
                        ...
                        <div id="block-field-form" class="disable-block">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-6 {{ $errors->has('cap') ? ' has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" name="cap" placeholder="Enter Cap" value="{{ old('cap') ? : $data->cap }}" required>
                                    @if ($errors->has('cap'))
                                        <span class="help-block">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-6 {{ $errors->has('cap_reset') ? ' has-error' : '' }}">
                                    ...
                                    <select class="form-control" name="cap_reset">
                                        <option value="1" {{ (old('cap_reset') ? : $data->cap_reset) == 1 ? " selected" : "" }}>Yes</option>
                                        <option value="0" {{ (old('cap_reset') ? : $data->cap_reset) == 0 ? " selected" : "" }}>No</option>
                                    </select>
                            ...
                            <div class="form-group {{ $errors->has('reason') ? ' has-error' : '' }}">
                                ...
                                <textarea class="form-control" name="reason" rows="5" placeholder="">{!! old('reason') ? : $data->reason !!}</textarea>
...
<script>
        ...
        if(offerID) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('input[name=offer_name]').val(response.offer.campaign_name);
    ...
    function resetNetwork()
    {
        ...
        $('input[name=offer_name]').val('');

        $("input[name=everflow]").prop(false);
        $("input[name=ef_id]").val(0);
        $("input[name=ef_status]").val('');

        $("input[name=linktrust]").prop(false);
