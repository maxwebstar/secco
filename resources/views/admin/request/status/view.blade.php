            ...
            <div class="box box-default">
                ...
                <div class="box-body">
                    ...
                    <table class="table table-striped table-io-view">
                        ...
                        <tr><td>New LT Status</td><td>{{ $data->lt_status }}</td></tr>
                        <tr><td>New EF Status</td><td>{{ $data->ef_status }}</td></tr>
                        <tr><td></td><td>{{ $data->cap_reset ? "Yes" : "No" }}</td></tr>
                        <tr><td>Redirect Url</td><td>{{ $data->redirect_url }}</td></tr>
                ...
                <div class="box-footer">
                    ...
                    @switch($data->status)
                        @case(1)
                            <a type="button" class="btn btn-success" href="/admin/request/status/approve/{{ $data->id }}" onClick="return confirm('Approve ?')">Approve</a>
                            <button type="button" class="btn btn-danger" onClick="decline({{ $data->id }})">Decline</button>
                            <a type="button" class="btn btn-info" href="/admin/request/status/edit/{{ $data->id }}">Edit</a>
                            @break
                        @case(2)
                            <a type="button" class="btn btn-success" href="/admin/request/status/approve/{{ $data->id }}" onClick="return confirm('Approve ?')">Approve</a>
                            <a type="button" class="btn btn-info" href="/admin/request/status/edit/{{ $data->id }}">Edit</a>
                    ...
                    <a class="btn btn-default pull-right" href="{{ route('admin.request.status.index') }}" role="button" style="margin-left: 5px;">Back to List</a>
